...
TimeTable.prototype.addClocking = function (value) {
  minutes = this.textToMinutes(value)
  if (this.overrideLastEntry(minutes))
    this.clocking[this.count_clocking-1] = minutes
  else
    this.clocking[this.count_clocking++] = minutes
}
...
TimeTable.prototype.overrideLastEntry = function (minutes) {
  return  this.count_clocking > 0 && Math.abs(minutes - this.clocking[this.count_clocking-1]) < 2
}
