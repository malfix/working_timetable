...
MontlyTimeTable.prototype.toHtml = function (timetable){
	...
	for (var i = 0; i < this.total; i++){
	    ...
	    if (this.list[i].getDate().getDay() == 6 || this.list[i].getDate().getDay() == 0) {
			...
			html += "<tr><td>" + this.list[i].getDate().getDate() + "</td><td></td><td><td/><td></td><tr>";

		} else {
			html += "<tr><td>" + this.list[i].getDate().getDate() + "</td>" + this.h_td_colorize(deltaMinDay)  + this.h_td_colorize(deltaMinWeek) + '<td>' + this.list[i].validateClocking() + "</td><tr>";			
		}
	}
	return html;
}
