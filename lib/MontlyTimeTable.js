...
MontlyTimeTable.prototype.toHtml = function (timetable){
	...
	var deltaMinWeek = 0;
	for (var i = 0; i < this.total; i++){
		if (this.list[i].getDate().indexOf('LUN') !== -1) {
			deltaMinWeek = 0;
		}
	    var deltaMinDay = (this.list[i].getMinutesWorked() == 0 ) ? 0 : this.list[i].getMinutesWorked() - 480;
	    deltaMinWeek += deltaMinDay;
	    if (this.list[i].getDate().indexOf('SAB') !== -1 || this.list[i].getDate().indexOf('DOM') !== -1) {
			deltaMinWeek = 0;
			
			html += "<tr><td>" + this.list[i].getDate() + "</td><td></td><td><td/><td></td><tr>";

		} else {
			html += "<tr><td>" + this.list[i].getDate() + "</td>" + this.h_td_colorize(deltaMinDay)  + this.h_td_colorize(deltaMinWeek) + '<td>' + this.list[i].validateClocking() + "</td><tr>";			
		}
	}
	return html;
}
